import { Config } from '../globals/script/config';
import { now, pointerCoord } from './utils';
...
export function startTapClick(doc: Document, config: Config) {
  ...
  const useRippleEffect = config.getBoolean(config,'animated')&& config.getBoolean(config,'rippleEffect')
...
function getRippleEffect(el: HTMLElement) {
  if (el.shadowRoot) {
    const ripple = el.shadowRoot.querySelector('ripple-effect');
    if (ripple) {
      return ripple;
    }
  }
  return el.querySelector('ripple-effect');
}
